...
impl Primitive {
    pub fn draw(&self, ctx: &WebGlRenderingContext) {
        ...
        // Bind vertex buffer
        ctx.bind_buffer(GL::ARRAY_BUFFER, Some(&self.vertex_buffer));
        ...
        ctx.vertex_attrib_pointer_with_i32(2, 3, GL::FLOAT, false, 0, 0);

        // Bind color buffer
        ctx.bind_buffer(GL::ARRAY_BUFFER, Some(&self.color_buffer));
        ctx.vertex_attrib_pointer_with_i32(3, 4, GL::FLOAT, false, 0, 0);
        ...
        // Bind uv1 buffer
        ctx.bind_buffer(GL::ARRAY_BUFFER, Some(&self.uv0_buffer));
        ctx.vertex_attrib_pointer_with_i32(5, 2, GL::FLOAT, false, 0, 0);
